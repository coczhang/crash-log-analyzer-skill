#!/usr/bin/env bash
set -uo pipefail

VERSION="1.1.0"
PROCESS=""
APP_PATH=""
HOURS="24"
OUTDIR=""
REDACT=0
ZIP=0

usage() {
  echo "Usage: $0 [--process name] [--app /Applications/App.app] [--hours 24] [--out dir] [--redact] [--zip] [--version]"
}

while [ "$#" -gt 0 ]; do
  case "$1" in
    --process) PROCESS="${2:-}"; shift 2 ;;
    --app) APP_PATH="${2:-}"; shift 2 ;;
    --hours) HOURS="${2:-24}"; shift 2 ;;
    --out) OUTDIR="${2:-}"; shift 2 ;;
    --redact) REDACT=1; shift ;;
    --zip) ZIP=1; shift ;;
    --version) echo "$VERSION"; exit 0 ;;
    -h|--help) usage; exit 0 ;;
    *) echo "Unknown argument: $1" >&2; usage >&2; exit 2 ;;
  esac
done

if [ -z "$OUTDIR" ]; then
  OUTDIR="crash-info-macos-$(date +%Y%m%d-%H%M%S)"
fi

mkdir -p "$OUTDIR"
WARNINGS_FILE="$OUTDIR/collection-warnings.txt"
: > "$WARNINGS_FILE"

warn() {
  echo "$*" >> "$WARNINGS_FILE"
  echo "warning: $*" >&2
}

have_cmd() {
  command -v "$1" >/dev/null 2>&1
}

json_escape() {
  printf '%s' "$1" | sed 's/\\/\\\\/g; s/"/\\"/g'
}

hash_file() {
  if have_cmd shasum; then
    shasum -a 256 "$1" | awk '{print $1}'
  elif have_cmd sha256sum; then
    sha256sum "$1" | awk '{print $1}'
  else
    echo ""
  fi
}

redact_file() {
  file="$1"
  case "$file" in
    *manifest.json|*.zip|*.dmp|*.dump|*.core) return ;;
  esac
  tmp="${file}.redact.$$"
  if sed -E \
    -e 's#/(Users|home)/[^/[:space:]]+#/\1/USER_REDACTED#g' \
    -e 's#([0-9]{1,3}\.){3}[0-9]{1,3}#IP_REDACTED#g' \
    -e 's#[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}#EMAIL_REDACTED#g' \
    -e 's#([Pp]assword|[Pp]asswd|[Tt]oken|[Ss]ecret|[Aa]pi[_-]?[Kk]ey|[Aa]uthorization)[[:space:]]*[:=][[:space:]]*(([Bb]earer|[Bb]asic)[[:space:]]+)?[^[:space:],;]+#\1=REDACTED#g' \
    "$file" > "$tmp"; then
    mv "$tmp" "$file"
  else
    rm -f "$tmp"
    warn "Could not redact $file"
  fi
}

write_manifest() {
  manifest="$OUTDIR/manifest.json"
  warning_count="$(wc -l < "$WARNINGS_FILE" | tr -d ' ')"
  {
    echo "{"
    echo "  \"schema_version\": 1,"
    echo "  \"script\": \"collect_macos_crash_info.sh\","
    echo "  \"script_version\": \"$(json_escape "$VERSION")\","
    echo "  \"platform\": \"macos\","
    echo "  \"collected_at\": \"$(json_escape "$(date -u +%Y-%m-%dT%H:%M:%SZ)")\","
    echo "  \"process\": \"$(json_escape "$PROCESS")\","
    echo "  \"app_path\": \"$(json_escape "$APP_PATH")\","
    echo "  \"hours\": \"$(json_escape "$HOURS")\","
    echo "  \"redacted\": $([ "$REDACT" -eq 1 ] && echo true || echo false),"
    echo "  \"zip_requested\": $([ "$ZIP" -eq 1 ] && echo true || echo false),"
    echo "  \"warnings\": ["
    first_warning=1
    while IFS= read -r warning; do
      [ -n "$warning" ] || continue
      if [ "$first_warning" -eq 0 ]; then
        echo ","
      fi
      first_warning=0
      printf '    "%s"' "$(json_escape "$warning")"
    done < "$WARNINGS_FILE"
    echo
    echo "  ],"
    echo "  \"warnings_file\": \"collection-warnings.txt\","
    echo "  \"warnings_count\": $warning_count,"
    echo "  \"files\": ["
    first=1
    while IFS= read -r file; do
      [ -f "$file" ] || continue
      rel="${file#$OUTDIR/}"
      [ "$rel" = "manifest.json" ] && continue
      size="$(wc -c < "$file" | tr -d ' ')"
      sha="$(hash_file "$file")"
      if [ "$first" -eq 0 ]; then
        echo ","
      fi
      first=0
      printf '    {"path": "%s", "bytes": %s, "sha256": "%s"}' "$(json_escape "$rel")" "$size" "$(json_escape "$sha")"
    done < <(find "$OUTDIR" -type f | sort)
    echo
    echo "  ]"
    echo "}"
  } > "$manifest"
}

create_zip() {
  zip_path="$OUTDIR.zip"
  rm -f "$zip_path"
  if have_cmd zip; then
    (cd "$(dirname "$OUTDIR")" && zip -qr "$(basename "$zip_path")" "$(basename "$OUTDIR")") || warn "zip command failed"
  elif have_cmd python3; then
    if ! python3 - "$OUTDIR" "$zip_path" <<'PY'
import os
import sys
import zipfile

source, target = sys.argv[1], sys.argv[2]
with zipfile.ZipFile(target, "w", zipfile.ZIP_DEFLATED) as archive:
    for root, _, files in os.walk(source):
        for name in files:
            path = os.path.join(root, name)
            archive.write(path, os.path.relpath(path, os.path.dirname(source)))
PY
    then
      warn "python3 zip creation failed"
    fi
  else
    warn "Cannot create zip: neither zip nor python3 is available"
  fi
}

for cmd in log find cp uname; do
  have_cmd "$cmd" || warn "Missing command: $cmd"
done
for optional in sw_vers codesign spctl xattr shasum; do
  have_cmd "$optional" || warn "Optional command missing: $optional"
done

{
  echo "collected_at=$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  echo "script_version=$VERSION"
  echo "host=$(hostname 2>/dev/null || true)"
  echo "process=$PROCESS"
  echo "app_path=$APP_PATH"
  echo "hours=$HOURS"
  echo "redacted=$REDACT"
  sw_vers 2>/dev/null || true
  uname -a 2>/dev/null || true
} > "$OUTDIR/summary.txt"

if [ -n "$PROCESS" ]; then
  log show --predicate "process == \"$PROCESS\"" --last "${HOURS}h" > "$OUTDIR/process-log.txt" 2>&1 || warn "log show failed for process $PROCESS"
else
  log show --last "${HOURS}h" > "$OUTDIR/system-log-window.txt" 2>&1 || warn "log show failed"
fi

for dir in "$HOME/Library/Logs/DiagnosticReports" "/Library/Logs/DiagnosticReports"; do
  if [ -d "$dir" ]; then
    if [ -n "$PROCESS" ]; then
      find "$dir" -maxdepth 1 -type f -name "${PROCESS}_*.crash" -mtime -2 -print -exec cp {} "$OUTDIR/" \; 2>/dev/null || true
      find "$dir" -maxdepth 1 -type f -name "${PROCESS}_*.ips" -mtime -2 -print -exec cp {} "$OUTDIR/" \; 2>/dev/null || true
    else
      find "$dir" -maxdepth 1 -type f \( -name "*.crash" -o -name "*.ips" \) -mtime -1 -print -exec cp {} "$OUTDIR/" \; 2>/dev/null || true
    fi
  fi
done > "$OUTDIR/copied-reports.txt"

if [ -n "$APP_PATH" ]; then
  codesign --verify --deep --strict --verbose=4 "$APP_PATH" > "$OUTDIR/codesign-verify.txt" 2>&1 || warn "codesign verification failed or app is unsigned"
  codesign -d --entitlements :- "$APP_PATH" > "$OUTDIR/entitlements.plist" 2>&1 || warn "could not collect entitlements"
  spctl --assess --verbose=4 "$APP_PATH" > "$OUTDIR/spctl.txt" 2>&1 || warn "spctl assessment failed"
  xattr -l "$APP_PATH" > "$OUTDIR/xattr.txt" 2>&1 || warn "xattr collection failed"
fi

if [ "$REDACT" -eq 1 ]; then
  while IFS= read -r file; do
    [ -f "$file" ] && redact_file "$file"
  done < <(find "$OUTDIR" -type f)
fi

write_manifest

if [ "$ZIP" -eq 1 ]; then
  create_zip
  write_manifest
fi

echo "Crash collection written to $OUTDIR"
if [ "$ZIP" -eq 1 ] && [ -f "$OUTDIR.zip" ]; then
  echo "Zip written to $OUTDIR.zip"
fi
